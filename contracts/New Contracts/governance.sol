...
// [FIX #9] Import SafeERC20 for safe token transfers
...
contract DOVGovernance is AccessControl, ReentrancyGuard, Pausable {
    // [FIX #9] Enable SafeERC20 for IERC20 operations
    ...
    struct Proposal {
        uint256 id;
        // [FIX #10] Removed secretHash field — secret word mechanism removed
    ...
    error NotAuthorized();
    // [FIX #10] Removed WrongSecret error — secret word mechanism removed
    error AlreadyClaimed();
    error NoReward();
    error DidNotVote();
    // [FIX #10] Removed EmptySecret error — secret word mechanism removed
    ...
    /// @dev Deployment flow: deploy Vittagems → deploy DOVGovernance → call this → call lockVGMGToken()
    ...
    function addMember(address account) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // [FIX #8] Only push to memberList if this address was never added before
        // If joinedAt was previously set (non-zero before we overwrote it above),
        // the address is already in the array from a prior addition.
        // We use a simple check: scan for existing entry to avoid duplicates.
    ...
    /// @notice Creates a new reserve-update proposal
    ...
    // [FIX #10] Removed secretWord parameter — no longer needed
    function createProposal(
        ...
        // [FIX #10] Removed secret word validation
        ...
        proposals[proposalCount] = Proposal({
            id: proposalCount,
            // [FIX #10] Removed secretHash from struct
            ipfsCid: ipfsCid,
    ...
    /// @dev Requires that the voting period has ended and quorum/approval are met
    /// @param proposalId ID of the proposal to execute
    // [FIX #10] Removed secretWord parameter — role-based access control is sufficient
    function executeReserveUpdate(
        ...
        // [FIX #10] Removed secret word hash verification — access control is the security boundary
    ...
    function claimReward(uint256 proposalId) external nonReentrant {
        ...
        // [FIX #9] Use safeTransfer instead of raw transfer() + require()
        vgmgToken.safeTransfer(msg.sender, amount);
    ...
    /// @param val New quorum in basis points (1–10000)
    ...
    /// @param val New threshold in basis points (1–10000)
    ...
    function withdrawVGMG(address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        ...
        // [FIX #9] Use safeTransfer instead of raw transfer() + require()
        vgmgToken.safeTransfer(to, amount);
    }
