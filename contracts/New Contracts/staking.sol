...
contract TokenStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    function stake(
        ...
        // [FIX #2] Check active stake count instead of lifetime array length
        // This allows users who have fully unstaked old positions to stake again
        if (_userAggregates[msg.sender].activeStakeCount >= MAX_STAKES_PER_USER) revert MaxStakesReached();
        ...
        // [FIX #5] Use balance-difference pattern to handle fee-on-transfer tokens
        // Same approach already used in depositRewardPool()
        uint256 balanceBefore = stakingToken.balanceOf(address(this));
        stakingToken.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 balanceAfter = stakingToken.balanceOf(address(this));
        uint256 actualReceived = balanceAfter - balanceBefore;

        // If the token has transfer fees, revert to prevent accounting mismatch
        if (actualReceived != _amount) revert TransferAmountMismatch();
    ...
    function _updateSingleTierPlan(
        ...
        uint256 oldRate = plan.rewardRate;
        uint256 oldDuration = plan.duration;
        // [FIX #6] Capture old minimum stake BEFORE overwriting storage
        uint256 oldMinStake = plan.minStakeAmount;
        ...
        // [FIX #6] Emit the actual old value instead of the new value for both params
        emit TierPlanUpdated(_tier, oldRate, _rewardRate, oldMinStake, _minStake, _isActive);
